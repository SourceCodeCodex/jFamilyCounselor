...
import ro.lrg.jfamilycounselor.plugin.impl.usedtypesalgorithm.assignments.model.pair.{InconclusiveTypePair, NewAssignmentsPairs, ResolvedConcreteTypePair, SAssignmentsPair}
            ...
            if (depth > MAX_DEPTH_THRESHOLD)
              ...
              p.derivationResult match {
                ...
                case InconclusiveTypePair(pair) =>
                  WorkListStatePartialResult(inconclusive = List(pair), alreadyDerived =  List(p))
              }
            //format: on
          }
          .toList
