  ...
  def derive(pair: AssignmentsPair): List[AssignmentsPair] = {
    ...
    if (e1.referenceVariable.declaringMethod == e2.referenceVariable.declaringMethod)
      for {
        ...
        (a1, a2) = (
          pair._1.copy(expression = call.argumentAt(e1.referenceVariable.declaringMethod.indexOf(e1.referenceVariable)), lastRecordedType = e1.referenceVariable.`type`.filterNot(_.isObjectType).getOrElse(pair._1.lastRecordedType)),
          pair._2.copy(expression = call.argumentAt(e2.referenceVariable.declaringMethod.indexOf(e2.referenceVariable)), lastRecordedType = e2.referenceVariable.`type`.filterNot(_.isObjectType).getOrElse(pair._2.lastRecordedType))
        )
      } yield AssignmentsPair(a1, a2, pair.depth + 1, AssignmentsEstimationConfig.MAX_COMBINATIONS)
      ...
      for {
        ...
        (a1, a2) = (
          pair._1.copy(expression = c1.argumentAt(e1.referenceVariable.declaringMethod.indexOf(e1.referenceVariable)), lastRecordedType = e1.referenceVariable.`type`.filterNot(_.isObjectType).getOrElse(pair._1.lastRecordedType)),
          pair._2.copy(expression = c2.argumentAt(e2.referenceVariable.declaringMethod.indexOf(e2.referenceVariable)), lastRecordedType = e2.referenceVariable.`type`.filterNot(_.isObjectType).getOrElse(pair._2.lastRecordedType))
        )
