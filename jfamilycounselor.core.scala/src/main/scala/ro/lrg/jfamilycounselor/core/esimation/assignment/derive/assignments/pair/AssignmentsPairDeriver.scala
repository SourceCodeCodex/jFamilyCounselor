...
import ro.lrg.jfamilycounselor.core.model.expression.{ParameterExpression, ThisExpression}
  ...
  def derive(pair: AssignmentsPair): DerivationResult = {
    ...
    if (_1.resolveConcreteType.isDefined && _2.resolveConcreteType.isDefined)
      ...
      val (inconclusive, newAssignmentsPairs) = {
        ...
        // CAUTION: There might be the case that by this kind of derivation (a.k.a. intra-derivation)
        // we might encounter expressions that have a concrete type. This case will be handles in the
        // DeriverDispatcher. Also, we might encounter UnknownExpression.
        val pairs = for {
