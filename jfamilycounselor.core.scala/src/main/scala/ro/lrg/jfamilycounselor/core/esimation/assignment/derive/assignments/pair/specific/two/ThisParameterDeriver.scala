      ...
      (a1, a2) = (
        call match {
          case _: Instantiation =>
            Assignment(pair._1.referenceVariable, CallExpression(call), t.referenceVariable.`type`.filterNot(_.isObjectType).getOrElse(pair._1.lastRecordedType))
          case c: MethodCall =>
            val expr = c.callExpression.getOrElse(ThisExpression(This(c.methodUnsafe.underlyingJdtObject.getDeclaringType)))
            Assignment(pair._1.referenceVariable, expr, t.referenceVariable.`type`.filterNot(_.isObjectType).getOrElse(pair._1.lastRecordedType))
        ...
        Assignment(
          ...
          p.referenceVariable.`type`.filterNot(_.isObjectType).getOrElse(pair._2.lastRecordedType)
        )
      )
