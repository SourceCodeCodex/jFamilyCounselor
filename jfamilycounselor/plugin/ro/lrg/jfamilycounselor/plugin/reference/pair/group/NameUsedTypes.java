...
import ro.lrg.jfamilycounselor.capability.specific.coverage.name.NameUsedTypesCapability;
...
public class NameUsedTypes implements IRelationBuilder<MTypesPair, MReferencesPair> {
	...
	if (pair._1 instanceof IType && pair._2 instanceof ILocalVariable)
	    usedTypes = NameUsedTypesCapability.usedTypesTP((Pair<IType, ILocalVariable>) pair);
	else if (pair._1 instanceof ILocalVariable && pair._2 instanceof ILocalVariable)
	    usedTypes = NameUsedTypesCapability.usedTypesPP((Pair<ILocalVariable, ILocalVariable>) pair);
