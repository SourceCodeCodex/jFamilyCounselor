...
import ro.lrg.jfamilycounselor.capability.specific.coverage.assignments.AssignmentsUsedTypesCapability;
...
@RelationBuilder
public class AssignemntsUsedTypes implements IRelationBuilder<MTypesPair, MReferencesPair> {
	...
	if (pair._1 instanceof IType t1 && pair._2 instanceof ILocalVariable param2) {
	    ...
	    usedTypes = AssignmentsUsedTypesCapability.usedTypes((Pair<IJavaElement, IJavaElement>) pair, new Pair<>(t1, t2.get()));
	    ...
	    usedTypes = AssignmentsUsedTypesCapability.usedTypes((Pair<IJavaElement, IJavaElement>) pair, new Pair<>(t1.get(), t2.get()));
	}
