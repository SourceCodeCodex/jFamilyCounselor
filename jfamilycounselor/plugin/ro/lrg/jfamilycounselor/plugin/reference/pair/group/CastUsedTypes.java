...
import ro.lrg.jfamilycounselor.capability.specific.coverage.cast.CastUsedTypesCapability;
...
public class CastUsedTypes implements IRelationBuilder<MTypesPair, MReferencesPair> {
	...
	if (pair._1 instanceof IType && pair._2 instanceof ILocalVariable)
	    usedTypes = CastUsedTypesCapability.usedTypesTP((Pair<IType, ILocalVariable>) pair);
	else if (pair._1 instanceof ILocalVariable && pair._2 instanceof ILocalVariable)
	    usedTypes = CastUsedTypesCapability.usedTypesPP((Pair<ILocalVariable, ILocalVariable>) pair);
