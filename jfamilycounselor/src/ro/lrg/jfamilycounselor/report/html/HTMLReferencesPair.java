...
public record HTMLReferencesPair(String referencesPair, Double apertureCoverage, Duration duration, List<String> usedTypes) implements HTMLRendable {
	...
	var usedTypesEntries = usedTypes.stream()
		.map(p -> {
		    return typesPairEntryTemplate
			    .replace("{TYPES_PAIR}", p);
		})
		.collect(Collectors.joining("\n"));

	var html = referencesPairEntryTemplate
		.replace("{USED_TYPES}", usedTypesEntries)
		.replace("{RANDOM_ID}", randomUUID.toString())
		.replace("{REFERENCES_PAIR}", referencesPair)
