...
public class ExportHFTCViewJob extends Job {
	...
	var pairToClients = relevantTypes.parallelStream().flatMap(t -> {
		...
		yield mType.relevantReferencesPairs().getElements().stream().flatMap(r -> {
		    ...
		    if (apertureCoverage <= APERTURE_COVERAGE_THRESHOLD)
			return usedTypes.get().getElements().stream().map(p -> sortTypeEntries(t, p.getUnderlyingObject()));
	...
	var clientsPairs = pairToClients.entrySet().parallelStream().collect(Collectors.toMap(
		e -> stringify(e.getKey()._1) + stringify(e.getKey()._2),
		e -> e.getValue()));
	...
	var subMonitor3 = SubMonitor.convert(monitor, "(3/4) Rebuilding Type Hierarchies Trees", typeHierarchiesMap.size());
	...
	var subMonitor4 = SubMonitor.convert(monitor, "(4/4) Exporting JSON of Correlated Types ", typeHierarchiesMap.size());
