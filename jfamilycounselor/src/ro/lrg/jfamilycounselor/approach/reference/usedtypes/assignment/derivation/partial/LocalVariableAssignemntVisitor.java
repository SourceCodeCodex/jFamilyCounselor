...
class LocalVariableAssignemntVisitor extends ASTVisitor {
	...
	public boolean visit(VariableDeclarationFragment node) {
		if (Optional.ofNullable(node.resolveBinding()).isPresent()
				&& iLocalVariable.equals(node.resolveBinding().getJavaElement()))
	...
	public boolean visit(Assignment node) {
		...
		if (left instanceof SimpleName sn
				&& Optional.ofNullable(sn.resolveBinding()).stream().anyMatch(b -> b.getKind() == IBinding.VARIABLE)) {
