...
public class ParameterParameterDerivation {
	...
	public static List<Supplier<Pair<Expression, Expression>>> derive(
			Pair<ILocalVariable, ILocalVariable> assignedParameters, boolean isInitial) {
		if (!(assignedParameters._1.getDeclaringMember() instanceof IMethod)
				|| !(assignedParameters._2.getDeclaringMember() instanceof IMethod)) {
			logger.severe(
					"Some of the provided 'parameters' do not have the declaring member a method. This cannot happen!");
			return List.of();
		}
		...
		if (m1.equals(m2)) {
			...
			return calls.parallelStream().map(callF -> {
				var call = callF.get();
			...
			}).filter(o -> o.isPresent()).map(o -> o.get()).map(p -> lazy(p)).toList();
			...
			return cartesianProduct(calls1Opt.get(), calls2Opt.get()).parallelStream().map(p -> {
				var call1 = p._1.get();
				var call2 = p._2.get();
	...
	private static boolean haveSameTargetObject(Expression invocation1, Expression invocation2) {
		...
		if (invocation1 instanceof SuperMethodInvocation si1) {
			if (invocation2 instanceof SuperMethodInvocation si2) {
				var dt1 = Optional.ofNullable(si1.resolveMethodBinding()).map(b -> (IMethod) b.getJavaElement())
						.map(m -> m.getDeclaringType().getFullyQualifiedName());
				var dt2 = Optional.ofNullable(si2.resolveMethodBinding()).map(b -> (IMethod) b.getJavaElement())
						.map(m -> m.getDeclaringType().getFullyQualifiedName());
		...
		if (invocation1 instanceof MethodInvocation mi1) {
			if (invocation2 instanceof MethodInvocation mi2) {
				var dt1 = Optional.ofNullable(mi1.resolveMethodBinding()).map(b -> (IMethod) b.getJavaElement())
						.map(m -> m.getDeclaringType().getFullyQualifiedName());
				var dt2 = Optional.ofNullable(mi2.resolveMethodBinding()).map(b -> (IMethod) b.getJavaElement())
						.map(m -> m.getDeclaringType().getFullyQualifiedName());
				...
				return ce1Opt.flatMap(ce1 -> ce2Opt.map(ce2 -> {
					try {
						if (ce1 instanceof ThisExpression t1 && ce2 instanceof ThisExpression t2)
							return Optional.ofNullable(t1.resolveTypeBinding()).stream()
									.anyMatch(b -> b.isEqualTo(t2.resolveTypeBinding()));
