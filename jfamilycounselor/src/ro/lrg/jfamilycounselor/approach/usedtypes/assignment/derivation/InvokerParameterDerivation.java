...
/**
 * Upon deriving a parameter, it also record the invokers (if any) of the calls
 * where the actual parameter is specified.
 * 
 * 
 * @author rosualinpetru
 *
 */
public class InvokerParameterDerivation {
    private InvokerParameterDerivation() {
    }
	...
	return callsOpt.get().parallelStream()
		.map(callF -> {
			...
			var invoker = switch (call.getNodeType()) {
			...
			default:
			    logger.severe("During InvokerParameter derivation, could not determine the invoker.");
			    yield Optional.<Expression>empty();
			};

			return Pair.of(invoker, arg);
