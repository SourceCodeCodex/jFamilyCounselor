...
public class CastsBasedApproach extends CorrelationEstimationUsedTypesApproach {
	...
	var enclosingT1CastsMethods = filterRelevantMethods(enclosingT1CastsMethodsOpt.get());
	var enclosingT2CastsMethods = filterRelevantMethods(enclosingT2CastsMethodsOpt.get());

	// if there are no casts of any of the analyzed type, then the types are not
	// correlated
	...
	try {
	    ...
	    var castsT1Opt = extractTypeCastsFromScope(t1, enclosingT1CastsMethods.stream().filter(m -> m.getDeclaringType().equals(t2)).toList());
	...
	try {
	    ...
	    var castsT2Opt = extractTypeCastsFromScope(t2, enclosingT2CastsMethods.stream().filter(m -> m.getDeclaringType().equals(t1)).toList());
	...
	try {
		...
		var t1CastedVariables = relevantCastsT1.stream()
			.map(CastsBasedApproach::extractCastedVariable)
			.filter(o -> o.isPresent())
			.map(o -> o.get())
			.toList();

		var t2CastedVariables = relevantCastsT2.stream()
			.map(CastsBasedApproach::extractCastedVariable)
			.filter(o -> o.isPresent())
			.map(o -> o.get())
			.toList();

		if (!t1CastedVariables.isEmpty() && !t2CastedVariables.isEmpty() && t1CastedVariables.stream().anyMatch(v -> !t2CastedVariables.contains(v)))
		    return true;
	    }
	...
	return enclosingCastsMethods.stream()
		.filter(m -> !m.getElementName().contains(Constants.EQUAL))
		.filter(m -> isMethodOverriding(m).orElse(false))
		.toList();
    }
	...
	return casts.stream()
		...
		.filter(ce -> isDowncast(ce).orElse(false) &&
			castsFieldOrParameter(ce) &&
			!isGuarded(ce))
		.toList();
    }
