...
public class TypeParametersUsedTypesApproach {
	...
	if (typeParametersPair._1 instanceof IType thiz && typeParametersPair._2 instanceof ITypeParameter typeParameter) {
	    var handledSuccessfully = ThisTypeParameterHandler.handle(thiz, typeParameter, state);
	    if (!handledSuccessfully)
		return Optional.empty();
	} else if (typeParametersPair._1 instanceof ITypeParameter typeParameter1 && typeParametersPair._2 instanceof ITypeParameter typeParameter2) {
	...
	var possibleTypes = jfamilycounselor.metamodel.factory.Factory.getInstance().createMTypeParametersPair(typeParametersPair).possibleTypes().getElements().stream().map(p -> p.getUnderlyingObject()).toList();
