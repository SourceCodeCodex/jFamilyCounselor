package ro.lrg.jfamilycounselor.capability.coverage.name;
...
public class LevenshteinUsedTypesCapability {
	...
	var possibleTypes = DistinctConcreteConeProductCapability.distinctConcreteConeProduct(iType1, iType2);
	...
	return Optional.of(possibleTypes.get().stream().filter(p -> {
	    ...
	    areCorrelatedCache.put(p.swap(), areCorrelated);
