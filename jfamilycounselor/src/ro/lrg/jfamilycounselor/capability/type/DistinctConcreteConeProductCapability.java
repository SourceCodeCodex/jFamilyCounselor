...
public class DistinctConcreteConeProductCapability {
	...
	public static Optional<List<Pair<IType, IType>>> distinctConcreteConeProduct(IType iType1, IType iType2) {
		...
		return intersectionOpt.flatMap(intersection -> c1Opt.flatMap(c1 -> c2Opt.map(
				c2 -> CommonOperations.cartesianProduct(c1.stream().filter(t -> !intersection.contains(t)).toList(),
