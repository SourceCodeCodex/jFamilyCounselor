package ro.lrg.jfamilycounselor.capability.reference;
		...
		.filter(p -> {
			...
			try {
			    return !(m1.isConstructor() && !m2.isConstructor() ||
				    ...
				    t1Opt.equals(t2Opt) ||
				    t1Opt.flatMap(t1 -> t2Opt.flatMap(t2 -> DistinctConcreteConeProductCapability.distinctConcreteConeProduct(t1, t2))).map(cp -> cp.isEmpty()).orElse(true));
			...
			return parameterType(param).flatMap(tp -> DistinctConcreteConeProductCapability.distinctConcreteConeProduct(tp, t)).map(cp -> !cp.isEmpty()).orElse(false);
