...
public class TypeReferenceCapability {
    ...
    public static Optional<List<Supplier<Type>>> extractTypeReferencesFromScope(IType iType, List<IMember> encolsingMembers) {
	return Optional.of(
		encolsingMembers.stream()
			.map(m -> Optional.ofNullable(m.getCompilationUnit()))
