...
public class TypeCastCapability {
	...
	public static Optional<List<Supplier<CastExpression>>> extractTypeCastsFromScope(IType iType,
	...
	public static Optional<Boolean> isDowncast(CastExpression castExpression) {
		try {
			var castTypeOpt = Optional.ofNullable(castExpression.getType().resolveBinding())
					.map(b -> (IType) b.getJavaElement());
			var castExpressionTypeOpt = Optional.ofNullable(castExpression.getExpression().resolveTypeBinding())
					.map(b -> (IType) b.getJavaElement());
			return castTypeOpt.flatMap(castType -> castExpressionTypeOpt
					.map(castExpressionType -> SubtypeCapability.isSubtypeOf(castType, castExpressionType)));
		} catch (Throwable e) {
			return Optional.empty();
		}
	}
