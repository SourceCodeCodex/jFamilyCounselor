...
import ro.lrg.jfamilycounselor.capability.common.parameter.ParameterTypeCapability;
import ro.lrg.jfamilycounselor.capability.coverage.assignment.AssignmentUsedTypesCapability;
...
public class AssignemntUsedTypes implements IRelationBuilder<MTypesPair, MReferencesPair> {
	...
	if (pair._1 instanceof IType t1 && pair._2 instanceof ILocalVariable param2) {
	    ...
	    if (t2.isEmpty())
		throw new IllegalStateException("Assignments used types computation for pair: " + mReferencesPair.toString() + " failed. Previous checks need to ensure that this situation cannot happen.");
