...
import ro.lrg.jfamilycounselor.capability.coverage.name.LevenshteinUsedTypesCapability;
...
@RelationBuilder
public class LevenshteinUsedTypes implements IRelationBuilder<MTypesPair, MReferencesPair> {
	...
	if (pair._1 instanceof IType && pair._2 instanceof ILocalVariable)
	    usedTypes = LevenshteinUsedTypesCapability.usedTypesThisParam((Pair<IType, ILocalVariable>) pair);
	else if (pair._1 instanceof ILocalVariable && pair._2 instanceof ILocalVariable)
	    usedTypes = LevenshteinUsedTypesCapability.usedTypesParamParam((Pair<ILocalVariable, ILocalVariable>) pair);
