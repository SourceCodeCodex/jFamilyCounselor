...
import ro.lrg.jfamilycounselor.approach.usedtypes.name.NameBasedLevenshteinApproach;
...
@RelationBuilder
public class NameBasedLevenshteinUsedTypes implements IRelationBuilder<MTypesPair, MReferencesPair> {
	...
	if (pair._1 instanceof IType thiz && pair._2 instanceof ILocalVariable param)
	    usedTypes = parameterType(param).flatMap(paramType -> NameBasedLevenshteinApproach.instance().usedTypes(thiz, paramType));
	else if (pair._1 instanceof ILocalVariable param1 && pair._2 instanceof ILocalVariable param2)
	    usedTypes = parameterType(param1).flatMap(t1 -> parameterType(param2).flatMap(t2 -> NameBasedLevenshteinApproach.instance().usedTypes(t1, t2)));

	if (usedTypes.isEmpty())
	    throw new IllegalStateException("Levenshtein used types computation for pair: " + mReferencesPair.toString() + " failed. Previous checks need to ensure that this situation cannot happen.");
