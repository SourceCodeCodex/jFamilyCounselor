	...
	public Group<MTypesPair> buildGroup(MReferencesPair mReferencesPair) {
		...
		if (pair._1 instanceof IType thiz && pair._2 instanceof ILocalVariable param)
			usedTypes = parameterType(param)
					.flatMap(paramType -> NameBasedLevenshteinApproach.instance().usedTypes(thiz, paramType));
		else if (pair._1 instanceof ILocalVariable param1 && pair._2 instanceof ILocalVariable param2)
			usedTypes = parameterType(param1).flatMap(t1 -> parameterType(param2)
					.flatMap(t2 -> NameBasedLevenshteinApproach.instance().usedTypes(t1, t2)));

		if (usedTypes.isEmpty() || usedTypes.get().isEmpty())
			throw new IllegalStateException("Name-based-levenshtein used types computation for pair: "
					+ mReferencesPair.toString() + " failed. The result should be a non-empty list.");
