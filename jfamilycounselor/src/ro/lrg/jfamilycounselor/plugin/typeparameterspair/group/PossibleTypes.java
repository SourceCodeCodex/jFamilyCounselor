...
public class PossibleTypes implements IRelationBuilder<MTypesPair, MTypeParametersPair> {
	...
	if (pair._1 instanceof IType thiz && pair._2 instanceof ITypeParameter typeParam) {
		...
		throw new IllegalStateException("The number of type bounds for: " + typeParam + " is different than 1. Previous checks need to ensure that this situation cannot happen.");
		...
		throw new IllegalStateException("The number of type bounds for: " + typeParam1 + " is different than 1. Previous checks need to ensure that this situation cannot happen.");

	    var bounds2 = typeParameterBoundsTypes(typeParam2);
	    if (bounds2.isEmpty() || bounds2.get().size() != 1)
		throw new IllegalStateException("The number of type bounds for: " + typeParam2 + " is different than 1. Previous checks need to ensure that this situation cannot happen.");
	...
	if (possibleTypes.isEmpty() || possibleTypes.get().isEmpty())
	    throw new IllegalStateException("Possible types computation for pair: " + mTypeParametersPair.toString() + " failed or resulted in an empty list. Previous checks need to ensure that this situation cannot happen.");
