...
public class UsedTypes implements IRelationBuilder<MTypesPair, MTypeParametersPair> {
	...
	public Group<MTypesPair> buildGroup(MTypeParametersPair mTypeParametersPair) {
		...
		if (usedTypes.isEmpty())
			throw new IllegalStateException("Type-parameters-based used types computation for pair: "
					+ mTypeParametersPair.toString() + " failed. The result should be a non-empty list.");

		// if type parameters based cannot find any used types, then consider all
		// possible types
		if (usedTypes.get().isEmpty())
			usedTypes = Optional.of(mTypeParametersPair.possibleTypes().getElements().stream()
