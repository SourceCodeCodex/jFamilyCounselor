...
import ro.lrg.jfamilycounselor.capability.specific.coverage.name.LevenshteinUsedTypesCapability;
...
public class LevenshteinUsedTypes implements IRelationBuilder<MTypesPair, MReferencesPair> {
	...
	if (pair._1 instanceof IType && pair._2 instanceof ILocalVariable)
	    usedTypes = LevenshteinUsedTypesCapability.usedTypesTP((Pair<IType, ILocalVariable>) pair);
	else if (pair._1 instanceof ILocalVariable && pair._2 instanceof ILocalVariable)
	    usedTypes = LevenshteinUsedTypesCapability.usedTypesPP((Pair<ILocalVariable, ILocalVariable>) pair);
